...
void drawUI(){
    if(gameState==STATE_MENU) {
        ...
        drawMultiMenuScreen(vhsTime);
    }
    else if(gameState==STATE_MULTI_HOST) {
        drawHostLobbyScreen(vhsTime,netMgr.getPlayerCount());
    }
    else if(gameState==STATE_MULTI_JOIN) {
        drawJoinMenuScreen(vhsTime);
    }
        ...
        if(isPlayerDead) {
            ...
            if(!coop.doorOpen){
                ...
                if(nearSwitch0 || nearSwitch1) {
                    drawText("HOLD SWITCH POSITION",-0.24f,-0.35f,1.4f,0.75f,0.8f,0.55f,0.85f);
                }
            }
            
            if(falseDoorTimer>0) {
                drawText("FALSE DOOR SHIFT",0.48f,0.67f,1.0f,0.9f,0.35f,0.25f,0.7f);
            }
            ...
            if(nearbyWorldItemId>=0){
                if(nearbyWorldItemType==0) {
                    drawText("[E] PICK BATTERY",-0.18f,-0.43f,1.4f,0.8f,0.8f,0.55f,0.8f);
                }
                else if(nearbyWorldItemType==1) {
                    drawText("[E] PICK MEDKIT",-0.16f,-0.43f,1.4f,0.8f,0.8f,0.55f,0.8f);
                }
                else if(nearbyWorldItemType==2) {
                    drawText("[E] PICK BAIT",-0.14f,-0.43f,1.4f,0.8f,0.8f,0.55f,0.8f);
                }
            }
            ...
            if(storyMgr.hasHallucinations()) {
                drawHallucinationEffect((50.0f-playerSanity)/50.0f);
            }
            ...
            if(multiState==MULTI_IN_GAME){
                for(int i=0;i<MAX_PLAYERS;i++){
                    if(i==netMgr.myId) continue;
                    if(!netMgr.players[i].active) continue;
                    if(!netMgr.players[i].hasValidPos) continue;
            ...
            const char* minimapText = minimapEnabled ? 
                "MINIMAP ON [F6/F7/HOME/M]" : "MINIMAP OFF [F6/F7/HOME/M]";
            drawText(minimapText,0.36f,-0.96f,0.95f,0.55f,0.7f,0.8f,0.68f);
        }
    }
}
        ...
        if(netMgr.entitySnapshotReceived){
            ...
            for(int i=0;i<netMgr.entitySnapshotCount;i++){
                ...
                if(e.type==ENTITY_STALKER){
                    e.speed=1.5f;
                    e.detectionRange=20.0f;
                    e.attackRange=1.0f;
                }
                else if(e.type==ENTITY_CRAWLER){
                    e.speed=5.0f;
                    e.detectionRange=15.0f;
                    e.attackRange=1.5f;
                    e.pos.y=-0.8f;
                }
                else if(e.type==ENTITY_SHADOW){
                    e.speed=0.5f;
                    e.detectionRange=8.0f;
                    e.attackRange=2.0f;
                }
