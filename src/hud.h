...
void drawUI() {
    if (gameState == STATE_MENU) {
    ...
    else if (gameState == STATE_KEYBINDS || gameState == STATE_KEYBINDS_PAUSE) {
        drawKeybindsMenu(gameState == STATE_KEYBINDS_PAUSE, menuSel, keybindCaptureIndex);
    }
        ...
        if (isPlayerDead) {
            ...
            if (!coop.doorOpen) {
                drawText("ACTION: HOLD 2 SWITCHES", 0.44f, 0.61f, 1.15f, 0.8f, 0.84f, 0.6f, 0.92f);
            ...
            if (multiState == MULTI_IN_GAME) {
                for (int i = 0; i < MAX_PLAYERS; i++) {
                    ...
                    if (!netMgr.players[i].active || !netMgr.players[i].hasValidPos) continue;
            ...
            const char* minimapText = minimapEnabled ? "MINIMAP ON [F6/F7/HOME/M]" : "MINIMAP OFF [F6/F7/HOME/M]";
