    ...
    for(int y = 0; y < sz; y++) {
        for(int x = 0; x < sz; x++) {
            ...
            if(type == 0) {
                ...
                if(y > sz * 0.75f) {
                    float wd = perlin(x * 0.1f, y * 0.05f, 3) * (y - sz * 0.75f) * 0.4f;
                    r -= wd * 0.8f;
                    g -= wd;
                    b -= wd * 0.6f;
                }
                
                float streak = sinf(x * 0.2f) * 5.0f;
                r += streak;
                g += streak * 0.9f;
                b += streak * 0.5f;
            }
                ...
                r = 135 + cp - dt + fib;
                g = 115 + cp - dt * 1.3f + fib;
                b = 75 + cp * 0.5f - dt * 0.7f + fib * 0.5f;
                
                if(perlin(x * 0.08f, y * 0.08f, 2) > 0.6f) {
                    r -= 25;
                    g -= 25;
                    b -= 15;
                }
                ...
                // Ceiling - realistic drop ceiling panels
                int ts = 64;
                float lx = (float)(x % ts);
                float ly = (float)(y % ts);
                
                // Soft edge shadow (AO) - darker near edges
                float dx = fminf(lx, ts - lx);
                float dy = fminf(ly, ts - ly);
                ...
                r = 230 - var - damage + detail + fiber;
                g = 218 - var * 1.05f - damage * 1.15f + detail + fiber * 0.9f;
                b = 175 - var * 0.7f - damage * 0.7f + detail * 0.7f + fiber * 0.6f;
                ...
                if(perlin(x * 0.12f, y * 0.12f, 2) > 0.65f) {
                    r -= 8;
                    g -= 10;
                    b -= 6;
                }
