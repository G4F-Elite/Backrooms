...
inline void drawText(const char* s, float x, float y, float sc, float r, float g, float b, float a=1.0f) {
    ...
    for(const char* p = s; *p && vc < 512*24 - 24; p++) {
        ...
        float t[24] = {
            cx, cy, u0, 1, cx+cw, cy, u1, 1, cx+cw, cy+ch, u1, 0,
...
inline void drawOrbitDots(float cx, float cy, float tm, float r) {
    for(int i = 0; i < 5; i++) {
        float ang = tm * 1.2f + i * 1.257f;
        float px = cx + cosf(ang) * r;
        float py = cy + sinf(ang) * r * 0.4f;
        float a = 0.3f + 0.3f * sinf(tm * 2.5f + i);
        drawText("*", px, py, 1.3f, 0.8f, 0.7f, 0.25f, a);
    }
}

inline void drawFloatingSyms(float tm) {
    const char* sy[] = {"~", "=", "+", "#", ".", ":"};
    for(int i = 0; i < 12; i++) {
        float yBase = -1.0f + fmodf(tm * 0.08f + i * 0.17f, 2.2f);
        float xPos = -0.92f + (i % 6) * 0.32f + sinf(tm * 0.7f + i) * 0.03f;
        float al = 0.06f + 0.04f * sinf(tm * 1.5f + i * 0.9f);
        drawText(sy[i % 6], xPos, yBase, 1.0f, 0.5f, 0.45f, 0.25f, al);
    }
}

inline void drawWaveBar(float y, float tm) {
    char bar[41];
    for(int i = 0; i < 40; i++) {
        float wave = sinf(tm * 2.0f + i * 0.3f);
        bar[i] = (wave > 0.5f) ? '=' : ((wave > 0) ? '-' : '.');
    }
    bar[40] = 0;
    float al = 0.08f + 0.04f * sinf(tm);
    drawTextCentered(bar, 0.0f, y, 0.8f, 0.4f, 0.35f, 0.2f, al);
}
...
inline void drawMenu(float tm) {
    ...
    drawFloatingSyms(tm);
    drawWaveBar(-0.75f, tm);
    drawWaveBar(0.7f, tm + 1.5f);
    
    // Decorative orbiting dots
    drawOrbitDots(0.0f, 0.52f, tm, 0.5f);
    
    // Spinning elements on sides
